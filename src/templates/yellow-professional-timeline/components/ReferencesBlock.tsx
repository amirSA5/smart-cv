      ...
      <div className={visible.length === 1 ? "grid gap-2" : "grid grid-cols-2 gap-2"}>
        {visible.map((entry) => (
          <article key={entry.id} className={`${subtleCardClass} px-2.5 py-1.5`}>
            <p className="text-[9.8px] font-black uppercase leading-[11.5px] text-black">
              {entry.name}
            </p>
            <p className="text-[8.6px] font-bold leading-[10.2px] text-neutral-700">
              {joinParts([entry.position, entry.company])}
            </p>
            {entry.phone ? (
              <p className="mt-0.5 text-[8.3px] font-semibold leading-[9.8px] text-neutral-500">
                <span className="font-black text-black">{phoneLabel}: </span>
                {entry.phone}
              </p>
            ) : null}
            {entry.email ? (
              <p className="text-[8.3px] font-semibold leading-[9.8px] text-neutral-500">
